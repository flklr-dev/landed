        ...
        <div className="w-full max-w-sm space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-ink-muted mt-1">
              No credit card required. Upgrade to Premium when you're ready.
            </p>
          </div>
          ...
          <ul className="space-y-1.5">
            {[
              'URL auto-extraction — paste a link, AI fills the rest',
              'Kanban board — Saved → Applied → Interview → Offer',
              'Dashboard with pipeline overview',
            ].map((item) => (
              <li key={item} className="flex items-start gap-2 text-sm text-ink-muted">
                <Check size={13} className="text-ink shrink-0 mt-0.5" />
                {item}
              </li>
            ))}
          </ul>
