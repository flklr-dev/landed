...
export default function BoardPage() {
  ...
  useEffect(() => {
    const handleAssistantJobChange = (event: Event) => {
      const detail = (event as CustomEvent<{ type: 'updated' | 'created'; job: Job }>).detail;
      if (!detail?.job) return;

      if (detail.type === 'created') {
        setJobs((current) =>
          current.some((job) => job.id === detail.job.id)
            ? current
            : [detail.job, ...current]
        );
        return;
      ...
      setJobs((current) =>
        current.map((job) => (job.id === detail.job.id ? detail.job : job))
      );
      setSelectedJob((current) =>
        current?.id === detail.job.id ? detail.job : current
      );
    };

    window.addEventListener('landed:job-changed', handleAssistantJobChange);
    return () => window.removeEventListener('landed:job-changed', handleAssistantJobChange);
  }, []);
